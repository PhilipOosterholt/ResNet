...
def track_scores(epoch, lr, acc1, acc5):
    ...
    name = args.arch + '_accuracy scores'
    os.chdir(args.data)

    if 'log' not in os.listdir():
        os.mkdir('log')

    os.chdir(args.data + '/log')

    if name not in os.listdir():
        df = pd.DataFrame(columns=['Learning Rate', 'Accuracy top 1', 'Accuracy top 5'])
    else:
        df = pd.read_pickle(name)

    df.loc[epoch] = [lr, round(acc1.item(),2), round(acc5.item(),2)]
    df.to_csv(os.path.join(args.data, 'log', name + '.csv'))
    df.to_pickle(os.path.join(args.data,'log', name))
